...
const generateData = (numSamples, numBumps) => {
  const data = new Array(numSamples)
  for (let i = 0; i < numSamples; ++i) data[i] = 0
...
const xScale = scaleLinear({ domain: [0, SAMPLES_PER_LAYER - 1] })
...
function StreamGraph({ width, height, animate = true }) {
  ...
  const layers = transpose(keys.map(() => generateData(SAMPLES_PER_LAYER, BUMPS_PER_LAYER)))
...
export default function StreamBackground() {
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <StreamGraph width={dims.width} height={dims.height} animate />
    </div>
  )
}
